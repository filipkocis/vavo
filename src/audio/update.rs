...
pub(crate) fn update_spatial_listeners(
    ...
    mut query: Query<
        (&GlobalTransform, &mut SpatialListener),
        (With<Camera>, Changed<GlobalTransform>),
    >,
) {
...
pub(crate) fn update_spatial_audio_tracks(ctx: &mut SystemsContext, mut query: Query<()>) {
    ...
    let mut moved_emitter_query = query
        .cast::<(EntityId, &GlobalTransform), (With<SpatialEmitter>, Changed<GlobalTransform>)>();
    for (id, transform) in moved_emitter_query.iter_mut() {
        if let Some(spatial_track) = audio.spatial_tracks.get_mut(&id) {
            spatial_track
                .track
                .set_position(transform.translation(), Tween::default());
            continue;
        }
    }

    // Apply emitter commands, and create spatial tracks if they don't exist yet
    let mut emitter_commands_query =
        query.cast::<(EntityId, &SpatialEmitter, &GlobalTransform), Changed<SpatialEmitter>>();
    ...
    for (id, emitter, transform) in emitter_commands_query.iter_mut() {
        ...
        let mut_emitter = mut_commands_query
            .get(id)
            .expect("SpatialEmitter should exist");
        if let Some(spatial_track) = audio.spatial_tracks.get_mut(&id) {
        ...
        let track_handle = audio
            .track
            .add_spatial_sub_track(listener_id, transform.translation(), builder)
