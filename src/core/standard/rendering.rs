...
fn main_render_system<'a>(
    graph_ctx: RenderGraphContext, 
    ctx: &mut SystemsContext, 
    mut query: Query<'a, (&'a EntityId, &'a Handle<Material>, &'a Handle<Mesh>, &'a GlobalTransform)>
) {
    // Render assets
    ...
    // Camera
    let mut camera_query = query.cast::<(&EntityId, &Camera), (With<Projection>, With<Transform>, With<Camera3D>)>(); 
    ...
    for (_, mat, mesh, global_transform) in query.iter_mut() {
    ...
    // Set transform storage
    let mut storage = ctx.resources.get_mut::<TransformStorage>().unwrap();
    storage.update(&transforms, ctx);
...
fn create_main_pipeline_builder(device: &wgpu::Device, color_format: TextureFormat) -> PipelineBuilder {
    ...
    Pipeline::build("main_pipeline")
        .set_bind_group_layouts(vec![material_layout, transform_layout, camera_layout])
