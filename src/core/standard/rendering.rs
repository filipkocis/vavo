...
pub fn standard_main_node(ctx: &mut SystemsContext) -> GraphNode {
    ...
    GraphNodeBuilder::new("main")
        ...
        .set_custom_system(CustomGraphSystem::new("main_render_system", main_render_system))
...
fn main_render_system(
    ...
    let (active_camera_id, active_camera) = match camera_query.iter_mut().into_iter().filter(|(_, c)| c.active).take(1).next() {
        Some(camera) => camera,
        None => return,
    };
    ...
    // Setup pipeline
    render_pass.set_pipeline(unsafe { &*graph_ctx.node }.data.pipeline.as_ref().expect("Pipeline should have been generated by now").render_pipeline());
